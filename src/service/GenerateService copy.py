...
class GenerateService:
    ...
    def generate(
        ...
        outline_system_prompt = """
            You are an AI language model tasked with generating an outline or table of contents for a new document based on the provided specifications.
            The document should be structured into three main sections: Analysis, Results, and Case Uses.
            The total length of the document must be 3 chapters, 70 pages, divided as follows:
            ...
            The structure should include the book title, and a list of chapters.
            Each chapter should have a chapter number, chapter title, start page, and total pages.
            Each chapter should also have a list of sections.
            Each section should have a section number, section title, start page, and total pages.
        ...
        prompt = outline_system_prompt.replace('pageAnalysis', str(pageAnalysis)) \
                            .replace('pageResult', str(pageResult)) \
                            .replace('pageUseCase', str(pageUseCase)) \
                            .replace('{******}', str(service)) \
                            .replace('_outline_result_', str(outline[service]['result'])) \
                            .replace('_outline_use_case_', str(outline[service]['use_case']))
        print(prompt)
        # Create Assistant
        assistant = self.client.beta.assistants.create(
            name="Business Analysis Assistant",
